...
class _Problem:
    ...
    def initial_loss(self, x, *yj):
        "Defines the PINN initial loss to train the NN"
        raise NotImplementedError
...
#        tc = torch.zeros_like(x[:,0:1])
#       # pdb.set_trace()
#        tc = torch.reshape(tc,(40,40,40))
#        tc[0:6,20,20] = 20.0
#        tc = torch.reshape(tc,(-1,))
        ...
        los_val = losses.l2_loss(physics_hx, 0) + losses.l2_loss(physics_hy, 0) + losses.l2_loss(physics_ez, 0) 
 #       print('phy loss={}'.format(los_val))
        return los_val

    def initial_condition(self, x, y):
        Ez = y[:,2:3]    	
        Ez0 = torch.reshape(Ez, (40,40,40))
        Ez0 = Ez0[:,:,0]
#        pdb.set_trace()
        m0 = m1 = 0; s0 = s1 = 0.2
        ...
        f = exp
        f = torch.reshape(f,(40,40,40))
        f = f[:,:,0]
        los = Ez0-f
        los_v = losses.l2_loss(torch.reshape(los,(-1,)), 0) / (1600.0)
#        print('init loss={}'.format(los_v))
        return los_v
...
#        return y_new, j1_hx, j2_hx, j0_hy, j2_hy, j0_ez, j1_ez, j2_ez
        # source apply on ez
...
#        y_new = t2 * y
        j1_hx_new = t2 * j1_hx# - s *f *(-xn1/s1)
        j2_hx_new = jt2 * Hx + t2 * j2_hx# - js * f

        j0_hy_new = t2 * j0_hy# - s *f * (-xn0/s0)
        j2_hy_new = jt2*Hy + t2 * j2_hy# - js * f

        j0_ez_new = t2 * j0_ez + s * f * (-2*xn0/s0)
        j1_ez_new = t2 * j1_ez + s * f * (-2*xn1/s1)
...
#        cood = x[:,0:1]
#        tim = x[:,1:2]
#        tc = torch.zeros_like(cood)
#        tc = torch.reshape(tc, (120,60))
#        tc[60,0:4] = 1.0
#        tc = torch.reshape(tc,(-1,))
        physics_hy = (j0_hy[:,0]) - j2_ez[:,0]# - tc# be careful to slice correctly (transposed calculations otherwise (!))
        physics_ez = (j0_ez[:,0]) - j2_hy[:,0]# + tc # be careful to slice correctly (transposed calculations otherwise (!))
...
#        s, js, jjs   = boundary_conditions.sigmoid_2(-x[:,1:2], -2*sd, 0.2*sd)# beware (!) this gives correct 2nd order gradients but negative 1st order (sign flip!)
...
#        pdb.set_trace()
    ...
    def boundary_condition(self, x, y, j2, jj0, jj2, sd):
        ...
        # Apply u = tanh^2((t-0)/sd)*NN + sigmoid((d-t)/sd)*exp( -(1/2)((x/sd)^2) )  ansatz
#         t1 = sd / c0
#         u = tanh^2(2.5*t / t1) * NN + exp(-(1/2)(1.5*t/t1)^2)
...
#        s, _, jjs   = boundary_conditions.sigmoid_2(-x[:,1:2], -2*sd, 0.2*sd)# beware (!) this gives correct 2nd order gradients but negative 1st order (sign flip!)
        s, _, jjs = boundary_conditions.exp_decay(x[:,1:2], sd)
